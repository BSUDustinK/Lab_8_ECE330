...
void Seven_Segment_Digit (unsigned char digit, unsigned char hex_char)
{
	...
	*******************************************************************************/
		...
		GPIOE->ODR &= ~(0x1 << (digit+8));

		GPIOE->ODR |= _7SEG[hex_char];
...
*******************************************************************************/
	...
	for (digit=0 ; digit<8 ; digit++)
	{
	   ...
	   Seven_Segment_Digit(digit, ((HexValue & ~(~(0xF<<(digit * 4))))) >>(digit*4));
	}
...
/**
  * @brief  This function is executed in case ferror occurrence.
  * @retval None
  */
